...
export async function POST(_request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const result = await markSalePaid({ saleId: id, diveCenterId: profile.diveCenterId, sellerId: profile.id });
  if (!result.length) {
    return NextResponse.json({ error: "La reserva no existe, ya está pagada o no te pertenece." }, { status: 404 });
  }
