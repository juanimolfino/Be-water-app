...
export const activities = pgTable("activities", {
  ...
  // Costo del proveedor por cantidad de personas (solo terceros), misma
...
export const agendaItems = pgTable("agenda_items", {
  ...
  customerName: text("customer_name"),
  customerPhone: text("customer_phone"),
  isWeTravelSale: boolean("is_we_travel_sale").default(false).notNull(),
  notes: text("notes"),
  active: boolean("active").default(true).notNull(),
  createdByUserId: uuid("created_by_user_id").references(() => users.id, { onDelete: "set null" }),
  createdAt: timestamp("created_at", { withTimezone: true }).defaultNow().notNull(),
...
// Un solo registro por (centro, día) marca que ese día está lleno.
// Existencia de la fila = bandera activa, sin columna booleana.
