...
export function ExpenseForm({ categories }: { categories: ExpenseCategory[] }) {
  ...
  return (
    <>
      <Button type="button" className="mb-6 hidden md:inline-flex" onClick={() => setOpen(true)}>
      ...
      {open ? <ExpenseFormDialog categories={categories} onClose={() => setOpen(false)} /> : null}
    </>
  );
}

export function ExpenseFormDialog({ categories, onClose }: { categories: ExpenseCategory[]; onClose: () => void }) {
    ...
    return (
      <div className="fixed inset-0 z-50 grid place-items-center bg-black/50 p-4" role="dialog" aria-modal="true">
        <div className="w-full max-w-md space-y-4 rounded-lg border bg-card p-6 shadow-lg">
          <p className="text-muted-foreground">Creá primero una categoría en Gastos para poder cargar gastos.</p>
